...
/**
 * Indicates whether the user's browser is IE11.  PTC code will treat IE11 as
 * a standards-conforming browser.  The Ext code will have 'isIE' set false
 * so that it does not use any special IE-only hacks.
 */
PTC_isIE11 = false;

/**
 * Indicates whether the user's browser is Edge. 
 */
PTC_isEdge = false;
...
(function(){
    var idSeed = 0,
        ...
        isIE = !isOpera && (check(/msie/) || check(/trident/)),// Backported from Extjs 4.2.2 to support IE11.
        isIE10 = isIE && check(/trident\/6/),
        isIE7 = isIE && !isIE10 && check(/msie 7/),
        isIE8 = isIE && !isIE10 && check(/msie 8/),
        isIE6 = isIE && !isIE7 && !isIE8,
        isGecko = !isWebKit && !isIE && check(/gecko/), // IE11 adds "like gecko" into the user agent string. Backported from Extjs 4.2.2
        ...
        // PTC ADDED
        // Treat IE11 like it is a standards-conforming browser (i.e. not IE!)
        if (isIE && (check(/msie 11/) || check(/trident\/7\.0/))) {
            isIE = false;
            PTC_isIE11 = true;
        }
    
        PTC_isEdge = check(/edge/);
...
Ext.lib.Event = function() {
    ...
    // private
    // PTC EXT OVERRIDE, added parameter skipScrollCheck to allow bypassing scroll checking in ie
    // for every event
    function getPageCoord (ev, xy, skipScrollCheck) {
        ev = ev.browserEvent || ev;
        var coord  = ev['page' + xy];
        if (!coord && coord !== 0) {
            coord = ev['client' + xy] || 0;

            if (Ext.isIE && skipScrollCheck !== true) {
    ...
    var pub =  {
        ...
        getPageX : function(ev, skipScrollCheck) {
            return getPageCoord(ev, "X", skipScrollCheck);
        },

        getPageY : function(ev, skipScrollCheck) {
            return getPageCoord(ev, "Y", skipScrollCheck);
        },


        getXY : function(ev, skipScrollCheck) {
            return [this.getPageX(ev, skipScrollCheck), this.getPageY(ev, skipScrollCheck)];
        },
